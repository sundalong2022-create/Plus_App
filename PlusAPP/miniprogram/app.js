...
const buildAuthErrorMessage = (error) => {
...
App({
    ...
    async ensureAuth(force = false) {
        ...
        authPromise = (async () => {
            const cachedToken = (0, auth_1.getAuthToken)();
            if (cachedToken) {
                try {
                    const me = await (0, auth_1.fetchCurrentUser)();
            ...
            const loginResult = await (0, auth_1.loginWithWeChat)();
            const me = await (0, auth_1.fetchCurrentUser)();
            this.globalData.authState = "ready";
            this.globalData.authMode = me.mode;
