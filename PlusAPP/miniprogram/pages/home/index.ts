...
const taskSummaryMap: Record<string, string> = {
...
const taskMetaMap: Record<string, string> = {
  review: "热身环节",
  learn: "理解环节",
  quiz: "熟练环节",
  level: "主游戏",
...
const statusMap: Record<TaskStatus, { text: string; tone: string; actionText: string }> = {
  todo: { text: "待开始", tone: "warm", actionText: "开始" },
  doing: { text: "进行中", tone: "sky", actionText: "继续" },
  done: { text: "已完成", tone: "mint", actionText: "查看" }
};

const buildTaskCards = (tasks: TaskItem[]) =>
  tasks.map((task) => ({
    ...task,
    statusText: statusMap[task.status].text,
    statusTone: statusMap[task.status].tone,
    actionText: statusMap[task.status].actionText,
    summary: taskSummaryMap[task.type] || "完成这一小步，再进入下一段练习。",
    meta: taskMetaMap[task.type] || "训练任务"
  }));

const buildStarItems = (stars: number[], currentDay: number, highlightDay = 0) =>
...
const buildCalendarItems = (stars: number[], currentDay: number, highlightDay = 0) =>
  stars.map((active, index) => ({
    label: `第${index + 1}天`,
...
const getNextTaskText = (taskCards: Array<{ title: string; status: TaskStatus }>): string => {
  const nextTask = taskCards.find((item) => item.status !== "done");
  return nextTask ? `下一步：${nextTask.title}` : "今天这轮训练已经完成。";
};

const getPrimaryActionText = (todayProgress: number): string => {
  if (todayProgress === 0) {
    return "开始今天的练习";
  }
...
const buildAuthView = (app: IAppOption) => {
...
const getRoute = (target: string, currentDay: number): { url: string; isTab?: boolean } => {
  switch (target) {
    case "wrongbook":
...
Page({
  ...
  handleTaskTap(event: { currentTarget: { dataset: { type: string } } }) {
  ...
  handleTileTap(event: { currentTarget: { dataset: { target: string } } }) {
  ...
  handlePreviewTap(event: { currentTarget: { dataset: { page: string } } }) {
