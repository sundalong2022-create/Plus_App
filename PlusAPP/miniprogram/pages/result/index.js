...
const getPerformanceMeta = (accuracy) => {
    if (accuracy >= 0.9) {
        return {
            badge: "这轮很稳",
            tone: "mint",
...
const buildCompletionCalendar = (stars, currentDay, highlightDay) => stars.map((active, index) => ({
    label: `第${index + 1}天`,
...
Page({
    ...
    async loadResult() {
        const [result, initData] = await Promise.all([request_1.api.session.latestResult(), request_1.api.app.init()]);
        const safeResult = result || initialResult;
        const performanceMeta = getPerformanceMeta(safeResult.accuracy);
        const highlightDay = safeResult.reward.type === "star"
        ...
        this.setData({
            ...
            completionCalendar: buildCompletionCalendar(initData.progress.stars, initData.progress.currentDay, highlightDay),
            completedDays: initData.progress.completedDays
        });
        audio_1.audioService.playRewardComplete();
